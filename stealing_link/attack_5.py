...
def load_data(train_path1, test_path1, train_path2, test_path2):
    ...
    x_test = []
    y_test = []
    train_data = open(train_path1).readlines() + open(
        test_path1).readlines()  # all row from dataset1
    # test_data = open(train_path2).readlines() + open(test_path2).readlines()
    # # all row from dataset2
    ...
    for row in train_data:
        ...
        # kl_divergence01 = np.array([KL(t0,t1), KL(r0,r1), KL(f0,f1)])
        # kl_divergence10 = np.array([KL(t1,t0), KL(r1,r0), KL(f1,f0)])
        ...
        # feature_metric_vec = get_metrics(f0, f1, metric_type, operator_func)
        line = np.concatenate(
...
with open("result/attack_5.txt", "a") as wf:
    wf.write(
        "%s,%s,%d,%0.5f,%0.5f,%0.5f,%s\n" %
        (dataset2, "attack5_transfer_metrics_target:%s_shadow:%s" %
